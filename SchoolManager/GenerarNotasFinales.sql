-- ============================================================
-- GENERAR NOTAS FINALES CON APROBADOS Y REPROBADOS
-- Fecha: 16 de Octubre, 2025
-- Descripción: Genera notas con distribución realista usando floor
-- ============================================================
...
-- ============================================================
-- PASO 2: Generar notas usando floor y módulo para distribución
-- ============================================================
...
SELECT 
    ...
    a.id,
    CASE 
        -- Usar el ID del estudiante para determinar la distribución
        WHEN (u.id::text::bit(32)::int % 100) < 10 THEN 0.5 + ((u.id::text::bit(32)::int % 100) / 100.0) * 1.0  -- 10% notas muy bajas (0.5-1.5)
        WHEN (u.id::text::bit(32)::int % 100) < 30 THEN 1.5 + ((u.id::text::bit(32)::int % 100) / 100.0) * 1.0  -- 20% notas bajas (1.5-2.5)
        WHEN (u.id::text::bit(32)::int % 100) < 60 THEN 2.5 + ((u.id::text::bit(32)::int % 100) / 100.0) * 1.0  -- 30% notas regulares (2.5-3.5)
        WHEN (u.id::text::bit(32)::int % 100) < 85 THEN 3.5 + ((u.id::text::bit(32)::int % 100) / 100.0) * 1.0  -- 25% notas buenas (3.5-4.5)
        ELSE 4.5 + ((u.id::text::bit(32)::int % 100) / 100.0) * 0.5  -- 15% notas excelentes (4.5-5.0)
    END as nota,
    NOW(),
    a.teacher_id,
...
AND u.status = 'active'
ORDER BY u.id, a.id
LIMIT 12000; -- Generar 12000 calificaciones
