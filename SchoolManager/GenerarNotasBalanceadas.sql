-- ============================================================
-- GENERAR NOTAS BALANCEADAS CON APROBADOS Y REPROBADOS
-- Fecha: 16 de Octubre, 2025
-- Descripción: Genera notas con distribución balanceada
-- ============================================================
...
-- ============================================================
-- PASO 2: Generar notas balanceadas
-- ============================================================
...
SELECT 
    ...
    a.id,
    CASE 
        -- 10% estudiantes con notas muy bajas (0.5-1.5) - REPROBADOS
        WHEN RANDOM() < 0.10 THEN 0.5 + (RANDOM() * 1.0)
        -- 20% estudiantes con notas bajas (1.5-2.5) - REPROBADOS
        WHEN RANDOM() < 0.30 THEN 1.5 + (RANDOM() * 1.0)
        -- 30% estudiantes con notas regulares (2.5-3.5) - MIXTO
        WHEN RANDOM() < 0.60 THEN 2.5 + (RANDOM() * 1.0)
        -- 25% estudiantes con notas buenas (3.5-4.5) - APROBADOS
        WHEN RANDOM() < 0.85 THEN 3.5 + (RANDOM() * 1.0)
        -- 15% estudiantes con notas excelentes (4.5-5.0) - APROBADOS
        ELSE 4.5 + (RANDOM() * 0.5)
    END as nota,
    NOW(),
    a.teacher_id,
...
AND u.status = 'active'
ORDER BY RANDOM()
LIMIT 10000; -- Generar 10000 calificaciones
